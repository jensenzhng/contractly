...
from converter import convertToTXT
...
def uploadPDF():
    try:
        ...
        print(file)
        destination = "/".join([target, 'contract.pdf'])
        file.save(destination) # saves request as pdf
        convertToTXT(destination) # converts pdf at destination to txt
        contract = open('./temp/contract.txt', 'r')
        task_thread = threading.Thread(target=predict, args=(questions, contract.read(), './model/roberta-base'))
        task_thread.start()
    except Exception as err:
        print(err)
